...
setInterval(function(){  
    if (localStorage.getItem("bool") == "true"){
        ...
        .then( function(response){ 
            ...
            response.text().then(function(text){ 
                ...
                parsed_list = JSON.parse(products_list_result)
                console.log(parsed_list)
                if (parsed_list.length != 0){
                    add_results(parsed_list[0]);
                }
 ...
 function get_results_once_page_opened(){
    console.log("I am in get_results_once_page_opened function")
    fetch("http://127.0.0.1:5001/Results")
    .then( function(response){
        console.log(response);
        response.text().then(function(text){
            // console.log(text)
            parsed_list = JSON.parse(text)
