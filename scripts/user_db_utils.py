...
def add_user(User_Name, Name_User, Email_Address):
    try:
        ...
        answer = "Account for username {} has been successfully created".format(User_Name)
        return answer
...
def update_user_name(User_ID, Old_User_Name, New_User_Name):
    result = False
    try:
        db_name = "CFG_Project"
        db_connection = _create_db_connection(db_name)
...
def update_user_email(User_ID, Old_User_Email, New_User_Email):
    ...
    try:
        ...
        if db_connection:
            ...
            cur.close()
    return result
#update_user_email(59,'sophie@hotmail.com','sophieg@hotmail.com')
